/**
 * 投票路由
 * 规则：每人每天最多 N 票（N 为配置值），每天每作品一票；按中国时区；总票数为总人次；取消投票仅能取消今天的票。
 */
...
// 投票：校验开放时间 + 每人每天最多投票数 + 当天每作品一票（中国时区）
router.post('/', (req, res) => {
  ...
  try {
    const result = db.transaction(() => {
      const userVoteCountToday = db.prepare('SELECT COUNT(*) AS c FROM votes WHERE user_id = ? AND vote_date = ?').get(user.userid, voteDate).c;
      if (userVoteCountToday >= maxVotes) {
      ...
      const existingToday = db.prepare('SELECT 1 FROM votes WHERE work_id = ? AND user_id = ? AND vote_date = ?').get(workId, user.userid, voteDate);
      if (existingToday) throw new Error('ALREADY_VOTED');
      try {
        db.prepare('INSERT INTO votes (work_id, user_id, vote_date, created_at) VALUES (?, ?, ?, ?)').run(workId, user.userid, voteDate, now);
      } catch (insertErr) {
    ...
    if (err.message === 'MAX_VOTES_REACHED') {
      return sendJson(res, createErrorResponse(`已达到每人每天最多投票数（${maxVotes}）`, 'MAX_VOTES_REACHED', 400));
    }
    ...
    if (err.message === 'ALREADY_VOTED') {
      return sendJson(res, createErrorResponse('今日已对该作品投过票', 'ALREADY_VOTED', 400));
    }
    throw err;
  }
});

// 取消投票：仅能取消今天投的票，且在投票开放时间内
router.delete('/', (req, res) => {
  try {
    ...
    const voteDate = getTodayDateChina();
    const result = db.prepare('DELETE FROM votes WHERE work_id = ? AND user_id = ? AND vote_date = ?').run(workId, user.userid, voteDate);
    const voteCount = db.prepare('SELECT COUNT(*) AS c FROM votes WHERE work_id = ?').get(workId).c;

    if (result.changes === 0) {
      return sendJson(res, createErrorResponse('只能取消今天投的票，未找到今日投票记录', 'NOT_FOUND', 404));
    }
...
router.get('/user/count', (req, res) => {
  ...
  const db = getDb();
  const voteDate = getTodayDateChina();
  const count = db.prepare('SELECT COUNT(*) AS c FROM votes WHERE user_id = ? AND vote_date = ?').get(user.userid, voteDate).c;
  sendJson(res, createSuccessResponse({ count }));
});

// 作品投票统计：总票数为总人次；hasVoted 表示今日是否已投（中国时区）
router.get('/stats', (req, res) => {
  ...
  if (user) {
    const voteDate = getTodayDateChina();
    const row = db.prepare('SELECT 1 FROM votes WHERE work_id = ? AND user_id = ? AND vote_date = ?').get(workId, user.userid, voteDate);
    hasVoted = !!row;
  }
